  ...
  try{
    ...
    */
    u.billing_required=
      u.trial_expired ||
      u.subscription_expired ||
      (["inactive","pending","cancelled","paused"].includes(u.organization_billing_status) && !(trialEndsAt && trialEndsAt>now));
...
async function requireAuth(req,res,next){
  ...
  if(u.billing_required && !u.is_super_admin){
    const allowed=req.path==="/assinatura" || req.path==="/api/me" || req.path==="/api/meta" || req.path.startsWith("/api/notifications") || req.path==="/api/auth/logout" || req.path.startsWith("/api/billing/");

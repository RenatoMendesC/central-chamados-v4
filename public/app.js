...
let me=null,meta={categories:[],departments:[],brandName:'Central de ServiÃ§os'},tickets=[],users=[],assets=[],articles=[];
...
const ticketStatus={open:'Aberto',progress:'Em atendimento',resolved:'Resolvido',closed:'Encerrado'},priorityLabel={low:'Baixa',medium:'MÃ©dia',high:'Alta',urgent:'CrÃ­tica'},impactLabel={low:'Baixo',medium:'MÃ©dio',high:'Alto'};
const assetStatus={active:'Em uso',maintenance:'ManutenÃ§Ã£o',stock:'Estoque',retired:'Baixado'},eventLabel={created:'Chamado criado',updated:'Chamado atualizado',commented:'InteraÃ§Ã£o registrada',attachment_added:'Anexo incluÃ­do'};
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const fmt=d=>d?new Date(d).toLocaleString('pt-BR',{dateStyle:'short',timeStyle:'short'}):'â€”',fmtShort=d=>d?new Date(d).toLocaleDateString('pt-BR'):'â€”';
async function api(url,opt={}){const tenantHeader=localStorage.getItem('activeOrganizationId');const r=await fetch(url,{headers:{'Content-Type':'application/json',...(tenantHeader?{'X-Organization-Id':tenantHeader}:{}),...(opt.headers||{})},...opt}),j=await r.json().catch(()=>({}));if(r.status===401){location.href='/';throw new Error('SessÃ£o encerrada.')}if(!r.ok)throw new Error(j.error||'Erro na operaÃ§Ã£o.');return j}
...
function applyPlanNavigation(){
  ...
  const restricted = ['/ativos', '/relatorios', '/historico'];
  ...
  document.querySelectorAll('.nav-group-title').forEach(title => {
    ...
    if(nav && nav.classList.contains('nav')){
      ...
      const visible = links.some(link => {
        const href = link.getAttribute('href');
        return !restricted.includes(href) || premium;
      });
...
async function loadMe(){
  ...
  if(me.isSuperAdmin&&localStorage.getItem('activeOrganizationId')){
    ...
    if(h&&!document.getElementById('tenantMode')){
      ...
      wrap.innerHTML=`<span><b>Modo suporte</b><small>${esc(meta.companyName||'Empresa selecionada')}</small></span><button class="btn secondary small" id="backPlatform">â† Voltar Ã  Plataforma</button>`;
      h.prepend(wrap);$('#backPlatform').onclick=()=>{localStorage.removeItem('activeOrganizationId');location.href='/plataforma'};
    }
  }
  if($('#meBox')){
    $('#meBox').innerHTML=`<button class="user-chip" id="userMenuBtn">${avatar(me)}<span><strong>${esc(me.name)}</strong><small>${me.isSuperAdmin?'Super Admin':roleLabel[me.role]}</small></span><b>âŒ„</b></button><div class="user-dropdown hidden" id="userDropdown"><div class="user-dropdown-head">${avatar(me,'md')}<div><strong>${esc(me.name)}</strong><small>@${esc(me.username)}</small></div></div>${me.isSuperAdmin?'<button id="platformMenu">GestÃ£o da Plataforma</button>':''}<button id="profileMenu">Meu perfil</button><button id="logoutBtn" class="danger-text">Sair da conta</button></div>`;
...
function ensureV9Navigation(){
  ...
  if(me?.role!=='requester'&&hasPlan('business')&&!sidebar.querySelector('a[href="/historico"]')){
    const title=document.createElement('div');title.className='nav-group-title v9-nav';title.textContent='GovernanÃ§a';
    const nav=document.createElement('nav');nav.className='nav v9-nav';
    nav.innerHTML='<a href="/historico"><span class="nav-glyph">â—·</span><span>HistÃ³rico</span></a>';
  ...
  if(!sidebar.querySelector('a[href="/assinatura"]')){
    ...
    const nav=document.createElement('nav');nav.className='nav v9-nav';
    nav.innerHTML='<a href="/assinatura"><span class="nav-glyph">â—‡</span><span>Assinatura</span></a>';
    bottom.before(title,nav);
  }
}
...
function showTrialWelcome(){
  ...
  /*
    Mostra apenas uma vez por sessÃ£o do navegador.
    Uma nova entrada/login em outra sessÃ£o poderÃ¡ exibir novamente.
  */
  ...
  const title=info.expired
    ...
    : 'Seu perÃ­odo de teste estÃ¡ ativo';

  const description=info.expired
    ? (
        isAdmin
          ? 'Para continuar utilizando todos os recursos da Central de ServiÃ§os, escolha um dos planos disponÃ­veis.'
          : 'O perÃ­odo de teste da sua empresa terminou. Entre em contato com o administrador para continuar utilizando a plataforma.'
      )
    : `VocÃª tem ${info.days} ${info.days===1?'dia restante':'dias restantes'} para explorar a Central de ServiÃ§os${info.end?` â€” seu teste vai atÃ© ${fmtShort(info.end)}`:''}.`;

  const bg=modal(`
    <div class="modal-head">
      <div>
        <span class="eyebrow">${info.expired?'PERÃODO ENCERRADO':'BEM-VINDO Ã€ CENTRAL'}</span>
        <h2>${title}</h2>
        <p>${description}</p>
      </div>
      <button class="close" data-close aria-label="Fechar">Ã—</button>
    </div>
    ...
    ${!info.expired?`
      <div style="margin:8px 0 4px;padding:16px 18px;border:1px solid var(--border);border-radius:14px;background:var(--panel2);">
        ...
        <span style="color:var(--muted);font-size:.92rem;">
          Aproveite para testar chamados, ativos, usuÃ¡rios, relatÃ³rios e os demais recursos disponÃ­veis.
        </span>
      </div>
    `:''}
...
async function initRegister(){const token=new URLSearchParams(location.search).get('invite')||'';if(!token){feedback('Para criar uma conta, solicite um link de convite ao administrador da sua empresa.','error');$('#registerForm').querySelector('button').disabled=true;return}try{const r=await api('/api/public/invite/'+encodeURIComponent(token));const box=$('#inviteCompany');if(box)box.innerHTML=`Convite para <strong>${esc(r.invite.organization_name)}</strong> Â· ${roleLabel[r.invite.role]||r.invite.role}`;if(r.invite.email&&$('#registerForm').email)$('#registerForm').email.value=r.invite.email}catch(err){feedback(err.message,'error');$('#registerForm').querySelector('button').disabled=true;return}$('#registerForm').onsubmit=async e=>{e.preventDefault();try{const d=Object.fromEntries(new FormData(e.target));d.inviteToken=token;const r=await api('/api/auth/register',{method:'POST',body:JSON.stringify(d)});feedback(r.message);setTimeout(()=>location.href='/',1200)}catch(err){feedback(err.message,'error')}}}
async function initDashboard(){await loadMe();const r=await api('/api/dashboard');const first=me.name.split(' ')[0];$('#welcomeTitle').textContent=`OlÃ¡, ${first}.`;$('#welcomeText').textContent=me.role==='requester'?'Acompanhe suas solicitaÃ§Ãµes e encontre respostas rapidamente.':'Acompanhe a saÃºde da operaÃ§Ã£o e priorize o que precisa de atenÃ§Ã£o agora.';const k=[['Backlog',Number(r.kpis.open)+Number(r.kpis.progress),'Chamados ativos','neutral'],['SLA vencido',r.kpis.overdue,'Exigem aÃ§Ã£o imediata','danger'],['Prioridade crÃ­tica',r.kpis.urgent,'Impacto elevado','warning'],['Resolvidos hoje',r.kpis.resolved_today,'Entregas concluÃ­das','success'],['Tempo mÃ©dio',`${r.kpis.avg_hours}h`,'AtÃ© resoluÃ§Ã£o','neutral'],['1Âª resposta',`${r.kpis.avg_first_response_min}m`,'MÃ©dia da operaÃ§Ã£o','neutral']];$('#dashKpis').innerHTML=k.map(x=>`<article class="metric-card ${x[3]}"><div class="metric-top"><span>${x[0]}</span><i></i></div><strong>${x[1]}</strong><small>${x[2]}</small></article>`).join('');$('#recentTickets').innerHTML=r.recent.length?r.recent.map(t=>`<a class="recent-row" href="/chamados?ticket=${t.id}"><span class="protocol">#${String(t.id).padStart(5,'0')}</span><div><strong>${esc(t.title)}</strong><small>${ticketStatus[t.status]} Â· ${priorityLabel[t.priority]}${t.assigned_name?` Â· ${esc(t.assigned_name)}`:''}</small></div><span class="sla ${isOverdue(t)?'late':''}">${slaText(t)}</span></a>`).join(''):'<div class="empty-mini">Nenhum chamado ainda.</div>';if($('#workload'))$('#workload').innerHTML=r.workload.length?r.workload.map(w=>`<div class="work-row"><span>${esc(w.name)}</span><div class="bar"><i style="width:${pct(w.total,Math.max(...r.workload.map(x=>x.total)))}%"></i></div><strong>${w.total}</strong></div>`).join(''):'<div class="empty-mini">Sem dados de carga.</div>';if($('#healthPanel')){const h=r.health||{};$('#healthPanel').innerHTML=`<div class="health-score"><div class="score-ring" style="--score:${Number(h.sla_compliance)||0}"><strong>${h.sla_compliance||0}%</strong><span>SLA</span></div><div><h3>SaÃºde operacional</h3><p>Indicador consolidado do cumprimento de SLA da operaÃ§Ã£o.</p></div></div><div class="mini-stats"><div><strong>${h.active_agents||0}</strong><span>Analistas ativos</span></div><div><strong>${h.assets||0}</strong><span>Ativos gerenciados</span></div><div><strong>${h.articles||0}</strong><span>Artigos publicados</span></div></div>`}$('#newTicketBtn')?.addEventListener('click',newTicket)}

function ticketRow(t){return `<article class="ticket-row ${isOverdue(t)?'overdue':''}" data-ticket="${t.id}">${avatar({name:t.requester_name,photo_data:t.requester_photo})}<div class="ticket-main"><strong>${esc(t.title)}</strong><small><span class="protocol">#${String(t.id).padStart(5,'0')}</span> Â· ${esc(t.requester_name)} Â· ${esc(t.category)}${t.asset_tag?` Â· ${esc(t.asset_tag)}`:''}</small></div><span class="priority ${t.priority}">${priorityLabel[t.priority]}</span><span class="status ${t.status}">${ticketStatus[t.status]}</span><span class="assignee">${t.assigned_name?esc(t.assigned_name):'NÃ£o atribuÃ­do'}</span><span class="sla ${isOverdue(t)?'late':''}">${slaText(t)}</span><span class="chevron">â€º</span></article>`}
async function loadTickets(){const p=new URLSearchParams();[['q','#search'],['status','#statusFilter'],['priority','#priorityFilter'],['category','#categoryFilter'],['assigned','#assignedFilter']].forEach(([k,s])=>{const v=$(s)?.value;if(v)p.set(k,v)});if($('#overdueFilter')?.checked)p.set('overdue','1');const r=await api('/api/tickets?'+p);tickets=r.tickets;$('#ticketCount').textContent=`${tickets.length} registro${tickets.length===1?'':'s'}`;$('#ticketList').innerHTML=tickets.length?tickets.map(ticketRow).join(''):'<div class="empty-state"><div class="empty-icon">â—‡</div><strong>Nenhum chamado encontrado</strong><span>Ajuste os filtros ou registre uma nova solicitaÃ§Ã£o.</span></div>';$$('[data-ticket]').forEach(x=>x.onclick=()=>openTicket(Number(x.dataset.ticket)))}
...
async function newTicket(){const assetOptions=(await loadAssetsForSelect()).map(a=>`<option value="${a.id}">${esc(a.asset_tag)} Â· ${esc(a.name)}</option>`).join('');const bg=modal(`<div class="modal-head"><div><span class="eyebrow">NOVA SOLICITAÃ‡ÃƒO</span><h2>Abrir chamado</h2><p>Registre o contexto completo para acelerar o atendimento.</p></div><button class="close" data-close>Ã—</button></div><form id="newTicketForm"><label class="field"><span>Assunto</span><input name="title" maxlength="180" placeholder="Ex.: Sistema financeiro nÃ£o abre" required></label><div class="form-grid"><label class="field"><span>Categoria</span><select name="category" required>${meta.categories.map(c=>`<option>${esc(c.name)}</option>`).join('')}</select></label><label class="field"><span>Prioridade</span><select name="priority"><option value="low">Baixa</option><option value="medium" selected>MÃ©dia</option><option value="high">Alta</option><option value="urgent">CrÃ­tica</option></select></label><label class="field"><span>Impacto</span><select name="impact"><option value="low">Baixo â€” uma pessoa</option><option value="medium" selected>MÃ©dio â€” equipe/setor</option><option value="high">Alto â€” operaÃ§Ã£o</option></select></label><label class="field"><span>Local</span><input name="location" placeholder="Sala, unidade, setor..."></label></div>${assetOptions?`<label class="field"><span>Ativo relacionado</span><select name="assetId"><option value="">Nenhum ativo</option>${assetOptions}</select></label>`:''}<label class="field"><span>DescriÃ§Ã£o detalhada</span><textarea name="description" rows="6" placeholder="Descreva o problema, quando comeÃ§ou e o que jÃ¡ foi testado." required></textarea></label><div class="modal-actions"><button type="button" class="btn ghost" data-close>Cancelar</button><button class="btn primary">Registrar chamado</button></div></form>`);$('#newTicketForm').onsubmit=async e=>{e.preventDefault();try{const r=await api('/api/tickets',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});bg.remove();toast(`Chamado #${r.id} criado.`);if(document.body.dataset.page==='tickets')loadTickets();else location.href=`/chamados?ticket=${r.id}`}catch(err){toast(err.message,'error')}}}
async function openTicket(id){const [r,ag,ast]=await Promise.all([api('/api/tickets/'+id),api('/api/users/agents'),loadAssetsForSelect()]),t=r.ticket,can=me.role!=='requester';const comments=r.comments.map(c=>`<div class="comment"><div>${avatar({name:c.author_name,photo_data:c.author_photo})}</div><div><div class="comment-head"><strong>${esc(c.author_name)}</strong><span>${roleLabel[c.author_role]||''}</span><time>${fmt(c.created_at)}</time></div><p>${esc(c.body).replace(/\n/g,'<br>')}</p></div></div>`).join('');const events=r.events.map(e=>`<div class="event"><i></i><div><strong>${eventLabel[e.event_type]||e.event_type}</strong><small>${esc(e.actor_name||'Sistema')} Â· ${fmt(e.created_at)}</small></div></div>`).join('');const files=r.attachments.map(a=>`<button class="attachment" data-attachment="${a.id}"><span>â–± ${esc(a.file_name)}</span><small>${Math.ceil(a.size_bytes/1024)} KB</small></button>`).join('');const bg=modal(`<div class="modal-head"><div><span class="eyebrow">CHAMADO #${String(t.id).padStart(5,'0')}</span><h2>${esc(t.title)}</h2><p>Aberto por ${esc(t.requester_name)} em ${fmt(t.created_at)}</p></div><button class="close" data-close>Ã—</button></div><div class="ticket-detail-grid"><main><div class="ticket-detail-badges"><span class="status ${t.status}">${ticketStatus[t.status]}</span><span class="priority ${t.priority}">${priorityLabel[t.priority]}</span><span class="impact">Impacto ${impactLabel[t.impact]||'MÃ©dio'}</span></div><section class="detail-box"><h3>DescriÃ§Ã£o</h3><div class="pre">${esc(t.description)}</div>${t.location?`<div class="detail-meta"><span>Local</span><strong>${esc(t.location)}</strong></div>`:''}${t.asset_tag?`<div class="detail-meta"><span>Ativo</span><strong>${esc(t.asset_tag)} Â· ${esc(t.asset_name)}${t.asset_model?` Â· ${esc(t.asset_model)}`:''}</strong></div>`:''}</section><section class="detail-box"><div class="detail-title"><h3>InteraÃ§Ãµes</h3><span>${r.comments.length}</span></div>${comments||'<div class="empty-mini">Nenhuma interaÃ§Ã£o registrada.</div>'}<form id="commentForm" class="comment-form"><textarea name="body" rows="3" placeholder="Registrar atualizaÃ§Ã£o, diagnÃ³stico ou orientaÃ§Ã£o..." required></textarea><button class="btn primary small">Enviar atualizaÃ§Ã£o</button></form></section><section class="detail-box"><div class="detail-title"><h3>Anexos</h3><label class="file-label">+ Anexar arquivo<input id="attachFile" type="file"></label></div>${files||'<div class="empty-mini">Nenhum anexo.</div>'}</section><section class="detail-box"><h3>Linha do tempo</h3>${events||'<div class="empty-mini">Sem eventos.</div>'}</section></main><aside><div class="detail-box sticky"><h3>GestÃ£o do chamado</h3><label class="field"><span>Status</span><select id="mStatus" ${can?'':'disabled'}>${Object.entries(ticketStatus).map(([k,v])=>`<option value="${k}" ${t.status===k?'selected':''}>${v}</option>`).join('')}</select></label><label class="field"><span>Prioridade</span><select id="mPriority" ${can?'':'disabled'}>${Object.entries(priorityLabel).map(([k,v])=>`<option value="${k}" ${t.priority===k?'selected':''}>${v}</option>`).join('')}</select></label><label class="field"><span>Impacto</span><select id="mImpact" ${can?'':'disabled'}>${Object.entries(impactLabel).map(([k,v])=>`<option value="${k}" ${t.impact===k?'selected':''}>${v}</option>`).join('')}</select></label><label class="field"><span>ResponsÃ¡vel</span><select id="mAssigned" ${can?'':'disabled'}><option value="">NÃ£o atribuÃ­do</option>${ag.users.map(a=>`<option value="${a.id}" ${Number(t.assigned_to)===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></label>${can?`<label class="field"><span>Ativo</span><select id="mAsset"><option value="">Nenhum</option>${ast.map(a=>`<option value="${a.id}" ${Number(t.asset_id)===Number(a.id)?'selected':''}>${esc(a.asset_tag)} Â· ${esc(a.name)}</option>`).join('')}</select></label><label class="field"><span>Local</span><input id="mLocation" value="${esc(t.location||'')}"></label>`:''}<div class="sla-box ${isOverdue(t)?'late':''}"><small>SLA de resoluÃ§Ã£o</small><strong>${slaText(t)}</strong><span>Prazo: ${fmt(t.due_at)}</span></div>${can?'<button class="btn primary btn-block" id="saveTicket">Salvar alteraÃ§Ãµes</button>':''}${me.role==='admin'?'<button class="btn danger btn-block" id="deleteTicket">Excluir chamado</button>':''}</div></aside></div>`,true);$('#commentForm').onsubmit=async e=>{e.preventDefault();await api(`/api/tickets/${id}/comments`,{method:'POST',body:JSON.stringify({body:e.target.body.value})});bg.remove();openTicket(id);if($('#ticketList'))loadTickets()};$('#saveTicket')?.addEventListener('click',async()=>{await api(`/api/tickets/${id}`,{method:'PATCH',body:JSON.stringify({status:$('#mStatus').value,priority:$('#mPriority').value,impact:$('#mImpact').value,assignedTo:$('#mAssigned').value||null,assetId:$('#mAsset')?.value||null,location:$('#mLocation')?.value||''})});bg.remove();toast('Chamado atualizado.');loadTickets()});$('#deleteTicket')?.addEventListener('click',async()=>{if(confirm('Excluir este chamado definitivamente?')){await api(`/api/tickets/${id}`,{method:'DELETE'});bg.remove();loadTickets()}});$('#attachFile').onchange=async e=>{try{const f=e.target.files[0],data=await fileToDataUrl(f);await api(`/api/tickets/${id}/attachments`,{method:'POST',body:JSON.stringify({fileName:f.name,mimeType:f.type,data})});bg.remove();openTicket(id)}catch(err){toast(err.message,'error')}};$$('[data-attachment]').forEach(b=>b.onclick=async()=>{const a=await api('/api/attachments/'+b.dataset.attachment),link=document.createElement('a');link.href=a.data;link.download=a.fileName;link.click()})}
...
async function loadAssets(){const r=await api('/api/assets?q='+encodeURIComponent($('#assetSearch')?.value||''));assets=r.assets;$('#assetCount').textContent=`${assets.length} ativo${assets.length===1?'':'s'}`;$('#assetsGrid').innerHTML=assets.length?assets.map(a=>`<article class="asset-card" data-asset="${a.id}"><div class="asset-card-top"><span class="asset-type">${esc(a.asset_type)}</span><span class="asset-state ${a.status}">${assetStatus[a.status]}</span></div><h3>${esc(a.name)}</h3><code>${esc(a.asset_tag)}</code><div class="asset-specs"><span>${esc([a.manufacturer,a.model].filter(Boolean).join(' ')||'Modelo nÃ£o informado')}</span><span>${esc(a.assigned_user_name||'Sem usuÃ¡rio vinculado')}</span><span>${esc(a.location||a.department||'Local nÃ£o informado')}</span></div><footer><span>${a.ticket_count} chamado(s)</span><button class="btn secondary small">Gerenciar</button></footer></article>`).join(''):'<div class="empty-state"><div class="empty-icon">â–±</div><strong>Nenhum ativo cadastrado</strong><span>Cadastre computadores, impressoras, dispositivos e outros itens.</span></div>';$$('[data-asset]').forEach(x=>x.onclick=()=>editAsset(Number(x.dataset.asset)))}
async function assetForm(a=null){const ur=await api('/api/users/active'),u=ur.users;const bg=modal(`<div class="modal-head"><div><span class="eyebrow">CMDB / ATIVOS</span><h2>${a?'Gerenciar ativo':'Cadastrar ativo'}</h2><p>Centralize patrimÃ´nio, vÃ­nculo, localizaÃ§Ã£o e histÃ³rico.</p></div><button class="close" data-close>Ã—</button></div><form id="assetForm"><div class="form-grid"><label class="field"><span>PatrimÃ´nio / TAG</span><input name="assetTag" value="${esc(a?.asset_tag||'')}" required></label><label class="field"><span>Nome do ativo</span><input name="name" value="${esc(a?.name||'')}" required></label><label class="field"><span>Tipo</span><input name="assetType" value="${esc(a?.asset_type||'Notebook')}" required></label><label class="field"><span>Status</span><select name="status">${Object.entries(assetStatus).map(([k,v])=>`<option value="${k}" ${a?.status===k?'selected':''}>${v}</option>`).join('')}</select></label><label class="field"><span>Fabricante</span><input name="manufacturer" value="${esc(a?.manufacturer||'')}"></label><label class="field"><span>Modelo</span><input name="model" value="${esc(a?.model||'')}"></label><label class="field"><span>NÂº de sÃ©rie</span><input name="serialNumber" value="${esc(a?.serial_number||'')}"></label><label class="field"><span>UsuÃ¡rio responsÃ¡vel</span><select name="assignedUserId"><option value="">NÃ£o vinculado</option>${u.map(x=>`<option value="${x.id}" ${Number(a?.assigned_user_id)===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></label><label class="field"><span>Setor</span><input name="department" value="${esc(a?.department||'')}"></label><label class="field"><span>LocalizaÃ§Ã£o</span><input name="location" value="${esc(a?.location||'')}"></label></div><label class="field"><span>ObservaÃ§Ãµes</span><textarea name="notes" rows="4">${esc(a?.notes||'')}</textarea></label><div class="modal-actions">${a&&me.role==='admin'?'<button type="button" class="btn danger" id="deleteAsset">Excluir</button>':''}<span class="grow"></span><button type="button" class="btn ghost" data-close>Cancelar</button><button class="btn primary">Salvar ativo</button></div></form>`);$('#assetForm').onsubmit=async e=>{e.preventDefault();await api(a?`/api/assets/${a.id}`:'/api/assets',{method:a?'PATCH':'POST',body:JSON.stringify(Object.fromEntries(new FormData(e.target)))});bg.remove();toast(a?'Ativo atualizado.':'Ativo cadastrado.');loadAssets()};$('#deleteAsset')?.addEventListener('click',async()=>{if(confirm('Excluir este ativo? O vÃ­nculo com chamados serÃ¡ removido.')){await api(`/api/assets/${a.id}`,{method:'DELETE'});bg.remove();loadAssets()}})}
...
async function loadKnowledge(){const r=await api('/api/knowledge?q='+encodeURIComponent($('#kbSearch')?.value||''));articles=r.articles;$('#kbGrid').innerHTML=articles.length?articles.map(a=>`<article class="kb-card" data-article="${a.id}"><div class="kb-meta"><span>${esc(a.category||'Geral')}</span>${!a.published?'<b>Rascunho</b>':''}</div><h3>${esc(a.title)}</h3><p>${esc(a.summary||a.content.slice(0,150))}</p><footer><span>${a.views} visualizaÃ§Ãµes</span><span>Atualizado ${fmtShort(a.updated_at)}</span></footer></article>`).join(''):'<div class="empty-state"><div class="empty-icon">â‰¡</div><strong>Nenhum artigo encontrado</strong><span>Documente soluÃ§Ãµes recorrentes e reduza chamados repetidos.</span></div>';$$('[data-article]').forEach(x=>x.onclick=()=>openArticle(Number(x.dataset.article)))}
async function articleForm(a=null){const bg=modal(`<div class="modal-head"><div><span class="eyebrow">BASE DE CONHECIMENTO</span><h2>${a?'Editar artigo':'Novo artigo'}</h2></div><button class="close" data-close>Ã—</button></div><form id="articleForm"><label class="field"><span>TÃ­tulo</span><input name="title" value="${esc(a?.title||'')}" required></label><div class="form-grid"><label class="field"><span>Categoria</span><input name="category" value="${esc(a?.category||'')}"></label><label class="field checkbox-field"><input name="published" type="checkbox" ${a?.published!==false?'checked':''}><span>Publicado</span></label></div><label class="field"><span>Resumo</span><textarea name="summary" rows="2">${esc(a?.summary||'')}</textarea></label><label class="field"><span>ConteÃºdo</span><textarea name="content" rows="12" required>${esc(a?.content||'')}</textarea></label><div class="modal-actions">${a?'<button type="button" class="btn danger" id="deleteArticle">Excluir</button>':''}<span class="grow"></span><button type="button" class="btn ghost" data-close>Cancelar</button><button class="btn primary">Salvar artigo</button></div></form>`,true);$('#articleForm').onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));d.published=e.target.published.checked;await api(a?`/api/knowledge/${a.id}`:'/api/knowledge',{method:a?'PATCH':'POST',body:JSON.stringify(d)});bg.remove();toast('Artigo salvo.');loadKnowledge()};$('#deleteArticle')?.addEventListener('click',async()=>{if(confirm('Excluir artigo?')){await api(`/api/knowledge/${a.id}`,{method:'DELETE'});bg.remove();loadKnowledge()}})}
async function openArticle(id){const {article:a}=await api('/api/knowledge/'+id),bg=modal(`<div class="modal-head"><div><span class="eyebrow">${esc(a.category||'BASE DE CONHECIMENTO')}</span><h2>${esc(a.title)}</h2><p>Por ${esc(a.author_name||'Equipe')} Â· atualizado ${fmt(a.updated_at)}</p></div><button class="close" data-close>Ã—</button></div><article class="article-body">${esc(a.content).replace(/\n/g,'<br>')}</article>${me.role==='admin'?'<div class="modal-actions"><button class="btn secondary" id="editArticle">Editar artigo</button></div>':''}`,true);$('#editArticle')?.addEventListener('click',()=>{bg.remove();articleForm(a)})}
  ...
  if($('#audit')){
    if(hasPlan('business')){
      try{
        ...
        $('#audit').innerHTML='<div class="empty-mini">NÃ£o foi possÃ­vel carregar a auditoria.</div>';
      }
    }else{
      $('#audit').innerHTML='<div class="empty-mini">HistÃ³rico de auditoria disponÃ­vel a partir do plano Business.</div>';
    }
  }
...
async function initReports(){await loadMe();const r=await api('/api/reports');const ks=[['Volume total',r.kpis.total,'Chamados registrados'],['Backlog',Number(r.kpis.open)+Number(r.kpis.progress),'PendÃªncias atuais'],['SLA vencido',r.kpis.overdue,'Fora do prazo'],['SLA cumprido',`${r.kpis.sla_compliance}%`,'Resolvidos no prazo'],['Tempo resoluÃ§Ã£o',`${r.kpis.avg_hours}h`,'MÃ©dia geral'],['1Âª resposta',`${r.kpis.avg_first_response_min}m`,'Tempo mÃ©dio']];$('#kpis').innerHTML=ks.map(x=>`<div class="metric-card"><div class="metric-top"><span>${x[0]}</span></div><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join('');function bars(id,data,key,label){const max=Math.max(1,...data.map(x=>Number(x.total)));$(id).innerHTML=data.map(x=>`<div class="bar-row"><div><span>${esc(label?label[x[key]]||x[key]:x[key]||'NÃ£o atribuÃ­do')}</span><strong>${x.total}</strong></div><div class="bar"><i style="width:${pct(x.total,max)}%"></i></div></div>`).join('')}bars('#statusBars',r.status,'status',ticketStatus);bars('#priorityBars',r.priority,'priority',priorityLabel);bars('#categoryBars',r.category,'category');bars('#agentBars',r.agents,'name');bars('#dailyBars',r.daily,'day');bars('#impactBars',r.impact,'impact',impactLabel);$('#aging').innerHTML=`<div class="aging-item"><strong>${r.aging.under24}</strong><span>AtÃ© 24h</span></div><div class="aging-item"><strong>${r.aging.h24_72}</strong><span>24â€“72h</span></div><div class="aging-item danger"><strong>${r.aging.over72}</strong><span>Acima de 72h</span></div>`}
    ...
    try{
      ...
      if(billingCurrent){
        billingCurrent.textContent=
          'NÃ£o foi possÃ­vel carregar as informaÃ§Ãµes da assinatura.';
      }
    }
  }
      ...
      btn.onclick=async()=>{
        ...
        try{
          ...
          if(
            ...
            throw new Error(
              'O Mercado Pago nÃ£o retornou a URL do checkout.'
            );
          }
          ...
          toast(
            err.message ||
            'NÃ£o foi possÃ­vel criar a assinatura.',
            'error'
          );
        }
      };
...
async function initSubscription(){
  ...
  $('#subscriptionSummary').innerHTML=`
    <article class="metric-card"><div class="metric-top"><span>Plano atual</span></div><strong>${esc(planNames[org.plan]||String(org.plan||'â€”').toUpperCase())}</strong><small>${plan.price?money(plan.price)+'/mÃªs':'Plano da empresa'}</small></article>
    <article class="metric-card"><div class="metric-top"><span>Status</span></div><strong>${esc(status)}</strong><small>${trial&&data.trialDaysRemaining!==null?`${data.trialDaysRemaining} dia(s) de trial restantes`:'SituaÃ§Ã£o da assinatura'}</small></article>
    <article class="metric-card"><div class="metric-top"><span>UsuÃ¡rios</span></div><strong>${data.usersUsed||0}/${org.user_limit||0}</strong><small>LicenÃ§as em utilizaÃ§Ã£o</small></article>
    <article class="metric-card"><div class="metric-top"><span>PrÃ³xima cobranÃ§a</span></div><strong style="font-size:17px">${org.next_payment_at?fmtShort(org.next_payment_at):'â€”'}</strong><small>${org.billing_email?esc(org.billing_email):'E-mail financeiro nÃ£o informado'}</small></article>`;

  $('#subscriptionDetails').innerHTML=`
    <div class="subscription-detail"><span>Empresa</span><strong>${esc(org.name||'â€”')}</strong></div>
    <div class="subscription-detail"><span>InÃ­cio na plataforma</span><strong>${fmtShort(org.created_at)}</strong></div>
    ...
    <div class="subscription-detail"><span>Ãšltimo pagamento</span><strong>${fmt(org.last_payment_at)}</strong></div>
    <div class="subscription-detail"><span>AtualizaÃ§Ã£o da cobranÃ§a</span><strong>${fmt(org.billing_updated_at)}</strong></div>
    <div class="subscription-detail"><span>ID da assinatura</span><strong><code>${esc(org.mp_subscription_id||'â€”')}</code></strong></div>`;

  $('#subscriptionPlans').innerHTML=plans.map(p=>`<article class="subscription-plan ${p.id===org.plan?'current':''}"><span class="eyebrow">${p.id===org.plan?'PLANO ATUAL':'PLANO'}</span><h3>${esc(p.name)}</h3><strong>${money(p.price)}<small>/mÃªs</small></strong><p>AtÃ© ${p.userLimit} usuÃ¡rios na empresa.</p><button class="btn ${p.id===org.plan&&org.billing_status==='authorized'?'secondary':'primary'} billing-v9-subscribe" data-plan="${p.id}" ${p.id===org.plan&&org.billing_status==='authorized'?'disabled':''}>${p.id===org.plan&&org.billing_status==='authorized'?'Plano atual':p.id===org.plan?'Assinar este plano':'Escolher plano'}</button></article>`).join('');
  ...
  $$('.billing-v9-subscribe').forEach(btn=>btn.onclick=async()=>{
    ...
    try{
      ...
      if(!r.checkoutUrl)throw new Error('O Mercado Pago nÃ£o retornou a URL do checkout.');
...
const activityLabels={
  ticket_created:'Chamado criado',ticket_updated:'Chamado alterado',ticket_deleted:'Chamado excluÃ­do',ticket_comment_added:'ComentÃ¡rio no chamado',
  asset_created:'Ativo cadastrado',asset_updated:'Ativo alterado',asset_deleted:'Ativo excluÃ­do',
  user_created:'UsuÃ¡rio criado',user_updated:'UsuÃ¡rio alterado',knowledge_created:'Artigo criado',knowledge_updated:'Artigo alterado',knowledge_deleted:'Artigo excluÃ­do',
  settings_updated:'ConfiguraÃ§Ãµes alteradas',billing_checkout_started:'Checkout iniciado',billing_authorized:'Assinatura autorizada',organization_created:'Empresa criada'
};
const activityTypeLabels={ticket:'Chamado',asset:'Ativo',user:'UsuÃ¡rio',knowledge:'Conhecimento',settings:'ConfiguraÃ§Ãµes',billing:'Assinatura',organization:'Empresa'};
function renderActivityDetails(log){
  const details=log.details||{};
  const changes=details.changes||{};
  const changeRows=Object.entries(changes).map(([field,v])=>`<div class="change-row"><strong>${esc(field.replaceAll('_',' '))}</strong><span>${esc(v?.from??'â€”')}</span><b>â†’</b><span>${esc(v?.to??'â€”')}</span></div>`).join('');
...
async function initActivity(){
  ...
  const load=async()=>{
    ...
    if($('#activityActor')&&$('#activityActor').options.length<=1)$('#activityActor').innerHTML='<option value="">Todos os usuÃ¡rios</option>'+r.actors.map(u=>`<option value="${u.id}">${esc(u.name)}</option>`).join('');
...
async function initPlatform(){
  ...
  const load=async()=>{
    ...
    $('#platformKpis').innerHTML=[['Empresas',sum.organizations,'Total de tenants'],['Ativas',sum.active,'Assinaturas ativas'],['Em trial',sum.trials,'PerÃ­odo de teste'],['Suspensas',sum.suspended,'Acesso bloqueado'],['MRR estimado',money(sum.mrr_cents),'Receita recorrente'],['Backlog global',sum.backlog,'Chamados em aberto']].map(x=>`<div class="metric-card"><div class="metric-top"><span>${x[0]}</span></div><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join('');
    $('#orgBody').innerHTML=data.organizations.length?data.organizations.map(o=>`<tr><td><div class="tenant-cell"><span class="tenant-avatar">${esc(o.name.slice(0,2).toUpperCase())}</span><div><strong>${esc(o.name)}</strong><small>${esc(o.slug)} Â· ${esc(o.billing_email||'sem e-mail financeiro')}</small></div></div></td><td><span class="pill">${esc(o.plan)}</span></td><td>${o.users}/${o.user_limit}</td><td>${o.tickets}</td><td>${o.backlog}</td><td><span class="status-dot ${o.status}">${o.status==='active'?'Ativa':o.status==='trial'?'Trial':'Suspensa'}</span></td><td>${o.trial_ends_at?fmtShort(o.trial_ends_at):'â€”'}</td><td><button class="btn secondary small" data-enter-org="${o.id}">Acessar</button> <button class="btn ghost small" data-edit-org="${o.id}">Gerenciar</button></td></tr>`).join(''):'<tr><td colspan="8"><div class="empty-state"><strong>Nenhuma empresa cadastrada</strong><span>Crie seu primeiro cliente SaaS.</span></div></td></tr>';
...
function editOrganization(o,reload){
  ...
  const bg=modal(`<div class="modal-head"><div><span class="eyebrow">GESTÃƒO DO TENANT</span><h2>${esc(o.name)}</h2><p>${esc(o.slug)} Â· criado em ${fmtShort(o.created_at)}</p></div><button class="close" data-close>Ã—</button></div><form id="editOrg"><div class="form-grid"><label class="field"><span>Nome</span><input name="name" value="${esc(o.name)}"></label><label class="field"><span>Plano</span><select name="plan">${['start','business','pro'].map(x=>`<option value="${x}" ${o.plan===x?'selected':''}>${x}</option>`).join('')}</select></label><label class="field"><span>Status</span><select name="status">${[['active','Ativa'],['trial','Trial'],['suspended','Suspensa']].map(([x,l])=>`<option value="${x}" ${o.status===x?'selected':''}>${l}</option>`).join('')}</select></label><label class="field"><span>Limite de usuÃ¡rios</span><input name="userLimit" type="number" value="${o.user_limit}"></label><label class="field"><span>Trial (dias)</span><input name="trialDays" type="number" min="0" max="90" value="${o.trial_days||0}"></label><label class="field"><span>Fim do trial</span><input name="trialEndsAt" type="date" value="${dateVal(o.trial_ends_at)}"></label><label class="field"><span>Fim da assinatura</span><input name="subscriptionEndsAt" type="date" value="${dateVal(o.subscription_ends_at)}"></label><label class="field"><span>E-mail financeiro</span><input name="billingEmail" type="email" value="${esc(o.billing_email||'')}"></label><label class="field"><span>Contato</span><input name="contactName" value="${esc(o.contact_name||'')}"></label><label class="field"><span>Telefone</span><input name="contactPhone" value="${esc(o.contact_phone||'')}"></label></div><label class="field"><span>ObservaÃ§Ãµes comerciais</span><textarea name="commercialNotes" rows="4">${esc(o.commercial_notes||'')}</textarea></label><div class="modal-actions"><button type="button" class="btn ghost" data-close>Cancelar</button><button class="btn primary">Salvar alteraÃ§Ãµes</button></div></form>`,true);
